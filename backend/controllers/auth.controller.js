...
} from "../services/twillowService.js";
  ...
  try {
    ...
    if (email) {
      ...
      if (
        ...
        return response(res, 400, "Invailid or expired otp");
      }
  ...
  try {
    ...
    if (!userId) {
      return response(
        res,
        404,
        "unathorization ! please login before access our app",
      );
    }
    const user = await User.findById(userId);
    if (!user) {
