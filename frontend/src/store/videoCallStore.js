...
const useVideoCallStore = create(
  subscribeWithSelector((set, get) => ({
    ...
    toggleVideo: () => {
      ...
      if (localStream) {
        const videoTract = localStream.getVideoTracks()[0];
        if (videoTract) {
          videoTract.enabled = !isVideoEnabled;
    ...
    toggleAudio: () => {
      ...
      if (localStream) {
        const audioTract = localStream.getAudioTracks()[0];
        if (audioTract) {
          audioTract.enabled = !isAudioEnabled;
